  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-stone-500 dark:text-stone-500">{userGamification.totalXP} XP</span>
      </div>
    )
  }

  return (
    <div className="xp-bar-container p-4 rounded-xl bg-stone-100 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-stone-500 dark:text-stone-400">{title}</span>
        </div>
        <div className="text-right">
          <span className="text-lg font-bold text-green-600 dark:text-lime-400">{userGamification.totalXP.toLocaleString()}</span>
          <span className="text-sm text-stone-500 dark:text-stone-500 ml-1">XP</span>
        </div>
      </div>
      ...
      <div className="flex justify-between mt-1.5 text-xs text-stone-500 dark:text-stone-500">
        <span>{progress}%</span>
